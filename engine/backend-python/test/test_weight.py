...
class TestSchema:
    ...
    def test_define_basic(self):
        schema = Schema("test").define(
            "embed", Source("model.embed_tokens.weight")
        )
    ...
    def test_load_single_tensor(self):
        ...
        schema = Schema("test").define(
            "embed", Source("model.embed.weight")
        )
    ...
    def test_load_per_layer_tensors(self):
        ...
        mock_tensors = {
            f"model.layers.{i}.weight": torch.randn(32, 32)
            for i in range(4)
        }
    ...
    def test_load_fused_tensors(self):
        ...
        schema = Schema("test").define(
            "fused",
            Source.fuse(["model.a", "model.b", "model.c"], dim=0)
        )
    ...
    def test_load_with_sharding(self):
        ...
        schema = Schema("test").define(
            "col_shard", Source("weight").shard("column")
        )
        ...
        # Row sharding (dim=1)
        schema2 = Schema("test").define(
            "row_shard", Source("weight").shard("row")
        )
