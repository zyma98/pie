...
async def launch_and_handle(client, args, program_hash):
    modified_question = f"{args.question} (run {random.randint(1, 1_000_000)})"
    instance_args = [
        "--question", modified_question,
        "--proposal-tokens", args.proposal_tokens,
        "--aggregation-tokens", str(args.aggregation_tokens),
    ]
...
async def main(args):
    ...
    async with PieClient(args.server_uri) as client:
        ...
        append_log('./logs/test_9_got_pie.json', {
...
if __name__ == "__main__":
    ...
    benchmark_group.add_argument("--program-name", type=str, default="graph_of_thought", help="Name of the WASM program file (without .wasm extension).")
    ...
    wasm_args_group.add_argument("--question", type=str, default="What is the significance of the number 42 in science and culture?", help="Base question to send to the WASM program.")
    wasm_args_group.add_argument("--proposal-tokens", type=str, default="4,32,16,8,4,16,3,32", help="Comma-separated list of max tokens for initial proposals.")
    wasm_args_group.add_argument("--aggregation-tokens", type=int, default=64, help="Max tokens to generate for aggregation steps.")
...
#
# --- ✅ Benchmark Complete ---
# Total Time Taken:       7989.12 milliseconds
# Throughput:             16.02 requests/second
# --------------------------
