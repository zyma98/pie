...
/// Arguments for the `pie-cli abort` command.
#[derive(Args, Debug)]
pub struct AbortArgs {
    ...
    /// Prefix or full UUID of the instance to terminate.
    pub instance_id_prefix: String,
}

/// Handles the `pie-cli abort` command.
///
/// This function:
...
/// 6. Terminates the instance if a unique match is found
/// 7. Reports errors if no match or multiple matches are found
pub async fn handle_abort_command(args: AbortArgs) -> Result<()> {
    ...
    let prefix = &args.instance_id_prefix;
    let matching_instances: Vec<_> = instances
        .iter()
        .filter(|instance| instance.id.starts_with(prefix))
        .collect();
