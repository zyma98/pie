...
/// Generate the WIT interface wrapper with user code embedded
fn generate_wrapper(user_bundle_path: &Path, output_path: &Path) -> Result<()> {
    // Read the bundled user code
    let user_code = fs::read_to_string(user_bundle_path)
        .with_context(|| format!("Failed to read user bundle from {}", user_bundle_path.display()))?;

    // Generate wrapper with user code embedded inside async function
    let wrapper_content = format!(r#"// Auto-generated by pie-cli build
// This wrapper provides the WIT interface for the inferlet

// Make all inferlet exports available as globals
import * as __inferlet__ from 'inferlet';
Object.assign(globalThis, __inferlet__);

// User code embedded in async function
