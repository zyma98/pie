...
class ClientConfig:
    ...
    def create(
        ...
        final_username = username or (config_file.username if config_file else None) or os.getlogin()
        
        # Get enable_auth setting
        enable_auth = (config_file.enable_auth if config_file and config_file.enable_auth is not None else True)
...
def attach_instance(client: PieClient, instance_id: str) -> Instance:
    ...
    return asyncio.get_event_loop().run_until_complete(client.attach_instance(instance_id))
...
def program_exists(client: PieClient, program_hash: str) -> bool:
    ...
    return asyncio.get_event_loop().run_until_complete(client.program_exists(program_hash))
