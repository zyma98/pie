...
use crate::{backend, batching_old, runtime, service};
use dashmap::DashMap;
use prost::Message;
use rand::Rng;
...
impl Batchable<BatchGroup> for Command {
    fn strategy(&self) -> Box<dyn BatchingStrategy> {
        match self {
            Command::GetInfo { .. } => batching_old::immediate(),
            Command::Allocate { .. } => {
            ...
            Command::Forward { .. } => {
                ...
                match *config {
                    BatchingStrategyConfiguration::TOnly { t } => batching_old::t_only(t),
                    BatchingStrategyConfiguration::KOnly { k } => batching_old::k_only(k, None),
                    BatchingStrategyConfiguration::KOrT { k, t } => batching_old::k_or_t(t, k, None),
                    BatchingStrategyConfiguration::Adaptive => {
                        Box::new(batching_old::ManualStrategy::new(
                            TRIGGERS.fill_block_trigger.clone(),
            ...
            Command::CopyKvPage { .. } => batching_old::eager(),
            Command::MaskKvPage { .. } => batching_old::eager(),
            Command::EmbedText { .. } => {
                //batching::t_only(Duration::from_micros(100))
                batching_old::eager()
            }
            ...
            Command::Synchronize { .. } => batching_old::eager(),
            Command::EmbedImage { .. } => batching_old::eager(),
            Command::DebugQuery { .. } => batching_old::eager(),
            Command::InitializeAdapter { .. } => batching_old::eager(),
            Command::UpdateAdapter { .. } => batching_old::eager(),
            Command::ForwardWithAdapter { .. } => Box::new(batching_old::ManualStrategy::new(
                TRIGGERS.forward_text_trigger.clone(),
                Duration::from_millis(5),
            )),
            _ => unreachable!(),
