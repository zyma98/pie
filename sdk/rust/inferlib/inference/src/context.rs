...
use crate::forward::{Distribution, KvPage};
use crate::models::{Model, ModelBorrow, Tokenizer};
use crate::queues::{Queue, WitDistribution};
...
inferlib_macros::wit_interface!(inference);
...
impl Context {
    ...
    pub(crate) fn decode_step_dist(&mut self, temperature: f32, top_k: Option<u32>) -> Distribution {
        assert!(
            !self.token_ids_pending.is_empty(),
            "Must have at least one seed token"
        );
    ...
    pub(crate) fn verify_draft(&mut self, draft_tokens: Vec<u32>, draft_pos_ids: Vec<u32>) -> Vec<u32> {
        assert!(
            !self.token_ids_pending.is_empty(),
            "Must have at least one seed token"
        );
    ...
    pub(crate) fn fork(&self) -> Context {
        let (
            ...
            new_pos_ids,
            new_mask_pending,
        ) = if self.kv_page_last_len == self.kv_page_size && !self.token_ids_pending.is_empty() {
    ...
    pub(crate) fn generate_async(&self, sampler: SamplerConfig, stop_config: StopConfig) -> GenerateFuture {
        GenerateFuture {
            context: self.clone(),
            sampler,
            stop_config,
