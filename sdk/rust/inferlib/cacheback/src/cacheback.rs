...
impl TwoLevelLruCache {
    ...
    fn put(&mut self, leader: Vec<u32>, follower: Vec<u32>) {
        if let Some(leader_idx) = self.cache.get_index_of(leader.as_slice()) {
            ...
            if let Some(follower_idx) = vcache.get_index_of(follower.as_slice()) {
                let last = vcache.len() - 1;
                vcache.move_index(follower_idx, last);
            } else {
...
impl CacheTable {
    pub(crate) fn new(
        ...
        Self {
            inner: TwoLevelLruCache::new(
                leader_capacity as usize,
                follower_capacity as usize,
                leader_len as usize,
                follower_len as usize,
            ),
            prev_window: vec![0u32; window_len],
        }
    }

    pub(crate) fn update(&mut self, context: Vec<u32>) {
        let total = self.inner.leader_len + self.inner.follower_len;
        ...
        if full.len() >= total {
            for i in 0..=full.len() - total {
                let leader = full[i..i + self.inner.leader_len].to_vec();
                let follower = full[i + self.inner.leader_len..i + total].to_vec();
                self.inner.put(leader, follower);
            }
        }
    ...
    pub(crate) fn draft(&mut self) -> DraftResult {
        let positions: Vec<u32> = (1..=self.inner.follower_len as u32).collect();
        let mut trie = TrieForest::new(1);

        let key = &self.prev_window[self.prev_window.len() - self.inner.leader_len..];
        if let Some(drafts) = self.inner.get(key) {
            for draft in &drafts {
                trie.insert(draft, &positions);
            }
        }

        let (tokens, positions) = trie.linearize();
        DraftResult { tokens, positions }
    }

    pub(crate) fn clear(&mut self) {
        self.inner.clear();
        for value in &mut self.prev_window {
            *value = 0;
        }
    }
}
