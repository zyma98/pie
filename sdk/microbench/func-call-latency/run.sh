...
bakery build --lib --world echo-provider \
    "$SCRIPT_DIR/inferlib-p2p-echo-callee" \
    -o "$PY_CALLEE_OUT/inferlib_p2p_echo_callee.wasm"
bakery build --lib --world echo-provider \
    "$SCRIPT_DIR/inferlib-r2p-echo-callee" \
    -o "$PY_CALLEE_OUT/inferlib_r2p_echo_callee.wasm"

echo "=== Building Python echo callers (P2R, P2P) ==="
ln -sfn "$ECHO_CALLEE_BINDINGS" "$P2R_SYMLINK"
ln -sfn "$ECHO_CALLEE_BINDINGS" "$P2P_SYMLINK"
cd "$MICROBENCH_DIR"
bakery build --inferlib "$REPO_ROOT/sdk/rust/inferlib" func-call-latency/inferlib-p2r-echo-caller \
    -o target/wasm32-wasip2/release/inferlib_p2r_echo_caller.wasm
bakery build --inferlib "$REPO_ROOT/sdk/rust/inferlib" func-call-latency/inferlib-p2p-echo-caller \
