...
impl ConstrainedSampler {
    ...
    pub fn sample(&self, token_ids: &[u32], probs: &[f32]) -> u32 {
        ...
        let sampled_token_id = if let Some(token) = best_token {
            token
        } else if let Some(token) = res.first_bit_set() {
            token as u32
        } else {
            return inner.eos_token_id;
        };
...
impl BytePairEncoder {
    fn new(
        ...
        let regex = Regex::new(pattern).expect("invalid split regex pattern");
        let special_regex = {
            let parts = special_tokens_encoder
                .keys()
                .map(|s| fancy_regex::escape(s))
                .collect::<Vec<_>>();
            Regex::new(&parts.join("|")).expect("invalid special token regex")
        };
        ...
        assert_eq!(
            encoder.len(),
            decoder.len(),
            "encoder and decoder must be of equal length; maybe you had duplicate token indices"
        );
