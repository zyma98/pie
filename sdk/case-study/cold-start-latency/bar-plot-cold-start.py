...
ROW1_CASES = [
    ("rust-mono-template-generation", "Rust Monolithic"),
    ("rust-static-template-generation", "Rust Static Composition"),
    ("rust-dynamic-template-generation", "Rust Dynamic Composition"),
]

ROW23_CASES = [
    ("python-full-dynamic-template-generation", "Python Self-contained"),
    ("python-factored-dynamic-template-generation", "Python Factored"),
]

ROW23_CASES_WM = [
    ("python-full-dynamic-watermarking-numpy", "Python Self-contained"),
    ("python-factored-dynamic-watermarking-numpy", "Python Factored"),
]
    ...
    for vi, (case_key, case_label) in enumerate(cases):
        ...
        for j in range(n_groups):
            ...
            ax.bar(
                ...
                color=darken(colors[vi]), edgecolor="gray", linewidth=0.3,
                hatch=hatches[vi],
            )
            ax.bar(
...
def plot(results: dict, sizes: dict[str, float], out_path: Path) -> None:
    fig, axes = plt.subplots(3, 1, figsize=(4.5, 4.4))
    ...
    def _build_legend(ax, cases, colors, hatches, columnspacing=2.0, legend_y=2.4):
        ...
        for vi, (case_key, label) in enumerate(cases):
            ...
            col0.append(mpatches.Patch(
                facecolor=colors[vi], edgecolor="gray", linewidth=0.3,
                hatch=hatches[vi], label=label,
            ))
        col1: list[mpatches.Patch] = [
            mpatches.Patch(
                facecolor="#cccccc", edgecolor="gray", linewidth=0.3,
                label="Compile + Instantiate",
            ),
            mpatches.Patch(
                facecolor="#666666", edgecolor="gray", linewidth=0.3,
                label="Binary Upload",
            ),
        ]
