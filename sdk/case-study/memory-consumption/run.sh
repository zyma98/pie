...
ENGINE_LOG=$(mktemp)
ENGINE_PID=""
NUM_INSTANCES_RUST=2000
NUM_INSTANCES_PYTHON=720
NUM_TICK_RUST=100
NUM_TICK_PYTHON=36
...
print_fd_limit() {
    for pid in $(get_tree_pids "$ENGINE_PID"); do
        ...
        hard=$(awk '/^Max open files/ {print $5}' /proc/"$pid"/limits 2>/dev/null) || true
        local cmd
        cmd=$(ps -o args= -p "$pid" 2>/dev/null | head -c 60) || true
        printf "  PID %-8s  FD limit: soft=%s hard=%s  %s\n" "$pid" "${soft:-?}" "${hard:-?}" "${cmd:-<unknown>}"
    done
}
...
count_open_fds() {
    ...
    for pid in $(get_tree_pids "$ENGINE_PID"); do
        local count
        count=$(ls /proc/"$pid"/fd 2>/dev/null | wc -l) || true
        if [ -n "$count" ]; then
            total=$((total + count))
        fi
    done
...
for i in $(seq 1 $NUM_INSTANCES_PYTHON); do
    ...
    if (( i % NUM_TICK_PYTHON == 0 )); then
        ...
        fds=$(count_open_fds)
        echo "  submitted $i/$NUM_INSTANCES_PYTHON  RSS=${rss_kb} KB  FDs=${fds}"
    fi
done
...
for i in $(seq 1 $NUM_INSTANCES_RUST); do
    ...
    if (( i % NUM_TICK_RUST == 0 )); then
        rss_kb=$(measure_rss_kb)
        fds=$(count_open_fds)
        echo "  submitted $i/$NUM_INSTANCES_RUST  RSS=${rss_kb} KB  FDs=${fds}"
    fi
done
