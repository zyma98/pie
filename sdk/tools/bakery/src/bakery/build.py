...
def get_inferlib_bindings_wit_path(dep_name: str) -> Path:
    ...
    if pie_sdk := os.environ.get("PIE_SDK"):
        path = Path(pie_sdk) / "rust" / "inferlib" / bindings_dir / "wit"
        if path.exists():
            return path

    current_dir = Path.cwd()
    for parent in [current_dir] + list(current_dir.parents):
        wit_path = parent / "sdk" / "rust" / "inferlib" / bindings_dir / "wit"
        if wit_path.exists():
            return wit_path

    raise FileNotFoundError(
        f"Could not find bindings WIT for dependency '{dep_name}' "
        f"at sdk/rust/inferlib/{bindings_dir}/wit/. "
        "Please set PIE_SDK environment variable."
    )


def handle_python_build(
    input_path: Path, output: Path, debug: bool = False, inferlib: bool = False
) -> None:
    ...
    Args:
        ...
        debug: Enable debug mode.
        inferlib: Use inferlib WIT instead of inferlet.
    """
    # Check prerequisites
    ...
    with console.status("[bold green]Resolving paths...[/bold green]"):
        if inferlib:
            ...
            wit_paths = [get_inferlib_bindings_wit_path(d) for d in deps]
        else:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with console.status(
            ...
            generate_py_wrapper(entry_point, wrapper_py, package_name, inferlib=inferlib)
            ...
            run_componentize_py(
                wrapper_py, output, wit_paths, package_name, debug
            )
...
def handle_build_command(
    ...
    Args:
        ...
        inferlib: Use inferlib WIT instead of inferlet (Python only).
    """
    ...
    if platform == "rust":
        ...
        handle_python_build(input_path, output, debug, inferlib=inferlib)
    else:
