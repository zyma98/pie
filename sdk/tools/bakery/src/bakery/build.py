...
def read_pie_dependencies(project_dir: Path) -> list[str]:
    """Read the [dependencies] section from Pie.toml.

    Returns:
        List of dependency names (e.g. ["inferlib-inference", "inferlib-llguidance"]).
    """
    pie_toml_path = project_dir / "Pie.toml"
    if not pie_toml_path.exists():
        return []
    ...
    return list(pie_toml.get("dependencies", {}).keys())
...
def detect_platform(input_path: Path) -> str:
    ...
    if input_path.is_dir():
        ...
        if (input_path / "pyproject.toml").exists():
            return "python"
        # Check for main.py or app.py without pyproject.toml (simple Python project)
        if (input_path / "main.py").exists() or (input_path / "app.py").exists():
            return "python"
        raise ValueError(
...
def generate_py_wrapper(
    user_module: Path, output_path: Path, package_name: str, inferlib: bool = False
) -> None:
    ...
    Args:
        ...
        inferlib: If True, import inference_bindings instead of inferlet.
    """
    # Get the module name without extension
    module_name = user_module.stem

    if inferlib:
        wrapper_content = f"""# Auto-generated by bakery build --inferlib
...
# Patch before importing user module so 'from inference_bindings import set_return'
# picks up the tracking wrapper
...
def generate_dynamic_wit(
    ...
    Merges deps/ directories and import lines from one or more bindings
    WIT directories, then adds a world.wit with the dynamic run interface.

    Args:
        wit_paths: List of bindings WIT directories to merge.
    ...
    for wit_path in wit_paths:
        deps_src = wit_path / "deps"
        if deps_src.exists():
            copy_dir_recursive(deps_src, temp_wit_dir / "deps")

        source_world_wit = wit_path / "world.wit"
        if source_world_wit.exists():
            for line in source_world_wit.read_text().splitlines():
                ...
                if stripped.startswith("include ") or stripped.startswith("import "):
                    indented = f"    {stripped}"
                    if indented not in import_lines:
                        import_lines.append(indented)
...
def merge_dependency_wit(dep_wit_paths: list[Path], target_wit_dir: Path) -> None:
    """Merge dependency bindings WIT into an existing library WIT directory.

    For each dependency WIT path:
    - Copies its deps/ subdirectory into target_wit_dir/deps/
    - Extracts import/include lines from its world.wit

    Then injects collected import lines into the target world.wit
    (inside the ``world <name> {`` block).

    Args:
        dep_wit_paths: Resolved bindings WIT directories for each dependency.
        target_wit_dir: The library's wit/ directory (already copied to temp).
    """
    import re

    import_lines: list[str] = []

    for wit_path in dep_wit_paths:
        deps_src = wit_path / "deps"
        if deps_src.exists():
            copy_dir_recursive(deps_src, target_wit_dir / "deps")
    ...
    world_wit = target_wit_dir / "world.wit"
    content = world_wit.read_text()
    lines = content.splitlines()

    insert_idx = None
    for i, line in enumerate(lines):
        if re.match(r"\s*world\s+[\w-]+\s*\{", line):
            insert_idx = i + 1
            break

    if insert_idx is None:
        raise RuntimeError(
            f"Could not find 'world <name> {{' in {world_wit}. "
            "Cannot inject dependency imports."
        )

    for new_line in reversed(import_lines):
        lines.insert(insert_idx, new_line)

    world_wit.write_text("\n".join(lines) + "\n")
...
def run_componentize_py(
    ...
    generate_dynamic_wit(wit_paths, temp_wit_dir, package_name)
    ...
    for p in extra or []:
        cmd.extend(["-p", str(p.resolve())])
    cmd.extend(["-o", str(output_wasm_abs), module_name])
...
def get_inferlib_bindings_wit_path(dep_name: str, inferlib_dirs: list[Path]) -> Path:
    """Resolve a Pie.toml dependency name to its bindings WIT directory.

    Convention: ``inferlib-{name}`` maps to
    ``{dir}/{name}-bindings/wit/`` for the first matching directory.

    Args:
        dep_name: Dependency name from Pie.toml (e.g. "inferlib-inference").
        inferlib_dirs: Directories to search for the bindings WIT, in order.

    Raises:
        FileNotFoundError: If the WIT directory cannot be found in any directory.
    """
    component_name = dep_name.removeprefix("inferlib-")
    bindings_dir = f"{component_name}-bindings"
    searched = []
    for inferlib_dir in inferlib_dirs:
        path = inferlib_dir / bindings_dir / "wit"
        if path.exists():
            return path
        searched.append(str(path))

    raise FileNotFoundError(
        f"Could not find bindings WIT for dependency '{dep_name}'. "
        f"Searched: {', '.join(searched)}"
    )


def run_componentize_py_lib(
    wit_dir: Path,
    world: str,
    work_dir: Path,
    module_name: str,
    ...
    """Run componentize-py to compile a Python library component to WASM.

    Args:
        wit_dir: Path to the WIT directory defining the library's world.
        world: The WIT world name (e.g. "echo-provider").
        work_dir: Working directory containing Python source files.
        module_name: The Python module name to componentize (e.g. "app").
        output_wasm: Path to the output WASM file.
        extra: Additional Python package paths passed as -p to componentize-py.
    """
    output_wasm_abs = (
...
def handle_python_lib_build(
    ...
    Unlike application components, library components:
    - Export custom WIT interfaces (no run() wrapper)
    - Carry their own wit/ directory with a complete WIT world
    - Use a project-specific world name

    Args:
        input_path: Path to the project directory.
    ...
    if not wit_dir.exists():
        raise FileNotFoundError(
            f"Library component requires a wit/ directory in {project_dir}."
        )

    # Detect entry module: prefer app.py, then main.py
    app_py = project_dir / "app.py"
    main_py = project_dir / "main.py"
    if app_py.exists():
        entry_module = app_py
    elif main_py.exists():
        entry_module = main_py
    else:
        raise FileNotFoundError(f"No app.py or main.py found in {project_dir}.")

    module_name = entry_module.stem

    console.print("[bold]Building Python library component...[/bold]")
    console.print(f"   Input: [blue]{input_path}[/blue]")
    console.print(f"   Output: [blue]{output}[/blue]")
    console.print(f"   World: [dim]{world}[/dim]")
    console.print(f"   Module: [dim]{module_name}[/dim]")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with console.status(
            ...
            status.update("[bold green]Copying source files...[/bold green]")

            # Copy Python files
            for py_file in project_dir.glob("*.py"):
                shutil.copy2(py_file, temp_path / py_file.name)

            # Copy Python package subdirectories (dirs with __init__.py)
            ...
            # Merge dependency WIT if --inferlib is provided
            if inferlib:
                status.update("[bold green]Merging dependency WIT...[/bold green]")
                deps = read_pie_dependencies(project_dir)
                if deps:
                    dep_wit_paths = [
                        get_inferlib_bindings_wit_path(d, inferlib) for d in deps
                    ]
                    merge_dependency_wit(dep_wit_paths, temp_path / "wit")

            status.update(
                "[bold green]Compiling to WebAssembly component with componentize-py...[/bold green]"
            )
            run_componentize_py_lib(
                wit_dir=temp_path / "wit",
                world=world,
                work_dir=temp_path,
...
def handle_python_build(
    ...
    3. Find WIT paths (inferlib or inferlet)
    ...
    8. Copy library to temp directory (inferlet mode only)
    ...
    with console.status("[bold green]Resolving paths...[/bold green]"):
        if inferlib:
            deps = read_pie_dependencies(project_dir)
            wit_paths = [get_inferlib_bindings_wit_path(d, inferlib) for d in deps]
        else:
            inferlet_path = path_utils.get_inferlet_py_path()
            wit_paths = [get_inferlet_wit_path()]

    mode_label = "inferlib" if inferlib else "inferlet"
    console.print(f"[bold]🏗️  Building Python {mode_label}...[/bold]")
    console.print(f"   Input: [blue]{input_path}[/blue]")
    console.print(f"   Output: [blue]{output}[/blue]")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with console.status(
            ...
            generate_py_wrapper(
                entry_point, wrapper_py, package_name, inferlib=inferlib
            )
            ...
            if input_type == "file":
                ...
                # Also copy Python package subdirectories (dirs with __init__.py)
            ...
            if not inferlib:
                ...
                status.update(
                    "[bold green]📦 Bundling inferlet library...[/bold green]"
                )
...
def handle_build_command(
    ...
    Args:
        ...
        extra: Additional Python package paths passed as -p to componentize-py.
    """
    if lib and not world:
        raise ValueError("--world is required when using --lib.")

    if world and not lib:
        raise ValueError("--world can only be used with --lib.")
