...
fn parse_response(
    response: &str,
) -> std::result::Result<(Option<String>, Option<(String, String)>), String> {
...
fn divide_and_conquer<'a>(
    ...
    Box::pin(async move {
        ...
        if path.len() >= max_depth {
            ...
            let gen_future = solve_ctx.generate_async(SamplerConfig::Greedy, &stop_config);
            let response = poll_generate(&gen_future).await;
        ...
        let stop_config = make_stop_config(max_tokens as u32, eos_tokens);
        let gen_future = divide_ctx.generate_async(SamplerConfig::Greedy, &stop_config);
        let response = poll_generate(&gen_future).await;
        ...
        match parse_response(&response) {
            ...
            Ok((None, Some((task1, task2)))) => {
                ...
                let stop_config = make_stop_config(max_tokens as u32, eos_tokens);
                let gen_future = merge_ctx.generate_async(SamplerConfig::Greedy, &stop_config);
                let response = poll_generate(&gen_future).await;
