...
async fn main(mut args: Args) -> Result<()> {
    ...
    let extraction_ctx = Context::new(&model);
    ...
    let extraction_output = extraction_ctx.generate(SamplerConfig::Greedy, &stop_config);
    ...
    let query_ctx = Context::new(&model);
    ...
    let (_, all_facts) = futures::join!(poll_flush(&query_ctx), graph_work);
    ...
    let answer = query_ctx.generate(SamplerConfig::Greedy, &stop_config);
