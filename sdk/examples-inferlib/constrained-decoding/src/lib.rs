...
async fn main(mut args: Args) -> Result<()> {
    ...
    let escape_non_printable =
        model_name.starts_with("Qwen/Qwen3") || model_name.starts_with("deepseek-r1-distill-qwen-2");
    ...
    // Manual decode loop using decode_step_dist for grammar-constrained sampling.
    // Each step gets the full next-token distribution, applies the grammar mask
    // via the ConstrainedSampler, and feeds the chosen token back into the context.
    ...
    let output_token_ids = &generated_token_ids;
