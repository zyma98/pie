...
use inferlib_inference_bindings::{
    Context, Model, SamplerConfig, StopConfig, broadcast, subscribe,
};
...
async fn main(mut args: Args) -> Result<()> {
    ...
    let (user_prompt, accumulated_story) = if let Some(prev_topic) = config.prev_topic {
        let accumulated = subscribe(&format!("{}-{}", prev_topic, group_id));
        let prompt = format!(
    ...
    let stop_config = StopConfig {
        max_tokens: tokens_per_step as u32,
        eos_sequences: eos_tokens.clone(),
    };
    let contribution = ctx.generate(SamplerConfig::Greedy, &stop_config);
