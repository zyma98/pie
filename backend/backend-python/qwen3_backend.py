...
class Qwen3BackendAgent(BaseBackendAgent):
    """
    Qwen3-specific backend agent that handles Qwen3 model loading and ZMQ communication.
    """
    ...
    def __init__(self, model_name: str, ipc_endpoint: str, management_service_url: str,
        ...
        super().__init__(
            ...
            backend_type="qwen3",
            service_name=f"Qwen3Backend-{model_name.split('/')[-1]}",
    ...
    async def _load_model_impl(self, request: ModelLoadRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.isdir(local_dir):
                logger.info(f"Loading Qwen3 model from local cache: {local_dir}")
                model_path_or_name = local_dir
                local_only = True
            else:
                logger.info(f"Loading Qwen3 model from HuggingFace Hub: {model_to_load}")
            ...
            with warnings.catch_warnings():
                ...
                self.model = QwenForCausalLM.from_pretrained(
                    model_path_or_name,
            ...
            return {
                ...
                "device": self.device,
                "message": f"Qwen3 model {model_to_load} loaded successfully"
            }
    ...
    async def _unload_model_impl(self, request: ModelUnloadRequest) -> Dict[str, Any]:
        """Unload the current Qwen3 model."""
        try:
...
def main_run():
    ...
    parser.add_argument('--backend-api-port', type=int, default=8081,
    ...
    # Create and start the Qwen3 backend agent
    agent = Qwen3BackendAgent(
    ...
    if not agent.start():
        logger.error("Failed to start Qwen3 backend agent")
        return 1

    logger.info(f"Qwen3 backend agent started successfully")
    ...
    try:
        ...
        agent.stop()
        logger.info("Qwen3 backend shutdown complete")
