...
async def launch_and_handle(client, args, program_hash):
    instance_args = [
        ...
        "--sink-window", str(args.sink_window),
    ]
...
async def main(args):
    ...
    async with PieClient(args.server_uri) as client:
        ...
        append_log('./logs/test_15_attnsink_pie.json', {
...
if __name__ == "__main__":
    ...
    benchmark_group.add_argument("--program-name", type=str, default="attention_sink", help="Name of the WASM program file (without .wasm extension).")
    ...
    wasm_args_group.add_argument("--prompt", type=str, default="Explain the meaning of this number", help="Base prompt to send to the WASM program.")
    wasm_args_group.add_argument("--max-tokens", type=int, default=128, help="Argument for WASM program: max tokens to generate.")
    wasm_args_group.add_argument("--sink-size", type=int, default=32, help="Argument for WASM program: sink size")
    wasm_args_group.add_argument("--sink-window", type=int, default=16, help="Argument for WASM program: sink window")
...
#
# --- ✅ Benchmark Complete ---
# Total Time Taken:       911.54 milliseconds
# Throughput:             140.42 requests/second
# --------------------------
