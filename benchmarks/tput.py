...
async def run_benchmark(args):
    ...
    wasm_path = script_dir.parent / "std" / "text-completion" / "target" / "wasm32-wasip2" / "release" / "text_completion.wasm"
    
    if not wasm_path.exists():
        print(f"Error: WASM binary not found at {wasm_path}")
        print("Please run `cargo build --target wasm32-wasip2 --release` in `std/text-completion` first.")
        sys.exit(1)
    ...
    async with PieClient(args.server) as client:
        ...
        async def worker(worker_id):
            ...
            while not queue.empty():
                ...
                # Launch instance
                try:
                    instance = await client.launch_instance(program_hash, arguments=inferlet_args)
                    while True:
                        ...
                        elif event in (Event.Aborted, Event.ServerError, Event.OutOfResources):
                             print(f"[{worker_id}] Req {req_id} aborted/failed: {event} {msg}")
                             break
...
def main():
    ...
    parser.add_argument("--prompt", default="Write a short story about a robot.", help="Prompt to use")
