//! Library component that instruments the Pie runtime API.
//!
//! This component both imports and exports the Pie runtime API (`inferlet:core/runtime`),
//! acting as a middleware layer that intercepts runtime calls. The `get_version()` method
//! is instrumented to prefix the version string with "Instrumented: ", while all other
//! runtime methods are passed through unchanged.
//!
//! Used for testing runtime API instrumentation and verifying that multiple instrumentation
//! layers can be stacked through repeated linking.
