//! Demonstrates hierarchical attention prefill based on XML structure.
//!
//! This example parses an XML document and constructs a custom attention mask
//! that enforces hierarchical relationships during the forward pass. Tokens
//! can only attend to tokens within their own node or in parent nodes.

use inferlet::brle::Brle;
use inferlet::forward::Forward;
use inferlet::{Args, Context, Result, Tokenizer};
use std::time::Instant;

const HELP: &str = "\
Usage: hierarchical-attention [OPTIONS]

A program demonstrating hierarchical attention masking based on XML structure.
...
pub async fn prefill_with_hierarchical_attention(ctx: &mut Context, xml_text: &str) {
    ...
    for j in 0..num_tokens {
        let mut masked: Vec<bool> = vec![true; j + 1];
    ...
    p.input_tokens(&parsed.tokens, &position_ids);
    ...
    ctx.position_ids
        .extend(&position_ids[..position_ids.len() - 1]);
