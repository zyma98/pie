...
@contextmanager
def mock_plugin_registry(is_registered=True):
    """Test utility: Mock PluginRegistry for TensorRecording tests."""
    with patch('debug_framework.services.plugin_registry.PluginRegistry') as mock_registry:
        mock_registry_instance = MagicMock()
        mock_registry.return_value = mock_registry_instance
        mock_registry_instance.is_registered.return_value = is_registered
        yield mock_registry_instance


# Global patch for create_from_tensor tests
@patch('debug_framework.models.tensor_recording.TensorRecording._validate_backend_name')
def _patch_create_from_tensor(mock_validate):
    """Decorator to patch backend validation for create_from_tensor tests."""
...
class TestTensorRecording:
    ...
    def test_tensor_recording_import_succeeds(self):
        """Test that TensorRecording import succeeds (implementation complete)."""
        from debug_framework.models.tensor_recording import TensorRecording
        assert TensorRecording is not None
    ...
    def test_tensor_recording_creation(self):
        ...
        # Mock plugin registry and create temp file for path validation
        with patch('debug_framework.services.plugin_registry.PluginRegistry') as mock_registry:
            mock_registry_instance = MagicMock()
            mock_registry.return_value = mock_registry_instance
    ...
    def test_tensor_metadata_validation(self):
        ...
        # Mock plugin registry for the invalid test and create temp file
        with patch('debug_framework.services.plugin_registry.PluginRegistry') as mock_registry:
            mock_registry_instance = MagicMock()
            mock_registry.return_value = mock_registry_instance
            mock_registry_instance.is_registered.return_value = True
    ...
    def test_tensor_data_path_validation(self):
        ...
        # Mock plugin registry for backend validation
        with patch('debug_framework.services.plugin_registry.PluginRegistry') as mock_registry:
            mock_registry_instance = MagicMock()
            mock_registry.return_value = mock_registry_instance
            mock_registry_instance.is_registered.return_value = True
    ...
    def test_device_info_validation(self):
        ...
        # Mock plugin registry for metal backend too
        with patch('debug_framework.services.plugin_registry.PluginRegistry') as mock_registry:
            mock_registry_instance = MagicMock()
            mock_registry.return_value = mock_registry_instance
            mock_registry_instance.is_registered.return_value = True
        ...
        # Mock plugin registry for invalid test too
        with patch('debug_framework.services.plugin_registry.PluginRegistry') as mock_registry:
            mock_registry_instance = MagicMock()
            mock_registry.return_value = mock_registry_instance
            mock_registry_instance.is_registered.return_value = True
    ...
    @pytest.mark.skipif(not TENSORRECORDING_AVAILABLE, reason="TensorRecording not implemented")
    @patch('debug_framework.models.tensor_recording.TensorRecording._validate_backend_name')
    def test_tensor_data_storage_and_retrieval(self, mock_validate):
    ...
    @pytest.mark.skipif(not TENSORRECORDING_AVAILABLE, reason="TensorRecording not implemented")
    @patch('debug_framework.models.tensor_recording.TensorRecording._validate_backend_name')
    def test_compression_handling(self, mock_validate):
    ...
    @pytest.mark.skipif(not TENSORRECORDING_AVAILABLE, reason="TensorRecording not implemented")
    @patch('debug_framework.models.tensor_recording.TensorRecording._validate_backend_name')
    def test_offline_validation_workflow(self, mock_validate):
    ...
    @pytest.mark.skipif(not TENSORRECORDING_AVAILABLE, reason="TensorRecording not implemented")
    @patch('debug_framework.models.tensor_recording.TensorRecording._validate_backend_name')
    def test_storage_optimization(self, mock_validate):
